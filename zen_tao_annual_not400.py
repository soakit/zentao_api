        ...
        for project_id in [project_id]:  # 遍历1, 18, 27
            for condition in ranges:  # 循环区间
                ...
                for item in json_query_data['bugs']:
                    ...
                    # 400组：方全斌(fangqb)、丁玲(dingling)、丁孟蝶(dingmd)、郭莉(guoli)、唐晨湘(tangcx)、刘璇(liuxuan)
                    group400 = ['fangqb', 'dingling', 'dingmd', 'guoli', 'tangcx', 'liuxuan']
                    invalidBug = ['duplicate']
                    
                    if item['openedBy'] not in group400 and item['resolution'] not in invalidBug:  
                        bug_list.append({
                            '编号': item['id'],
...
if __name__ == "__main__":
    # 创建参数解析器
    parser = argparse.ArgumentParser(description='非400组生产bug统计')
    ...
    title = '2026年6月15日-22日非400组生产bug'
