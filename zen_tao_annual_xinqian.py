        ...
        for project_id in [project_id]:  # 遍历1, 18, 27
            for condition in ranges:  # 循环区间
                ...
                for item in json_query_data['bugs']:
                    ...
                    # 新签pod：李奕鹏(liyp)、韦成双(weics)、钟超(zhongchao)、汤沃明(tangwm)、王勇圣(wangys)、王伟豪(wangwh)、吴之赛(wuzs)、袁阳(yuanyang)、简鑫凤(jianxf)、周梦成(zhoumc)、李龙学(lilx)
                    employees = ['liyp', 'weics', 'zhongchao', 'tangwm', 'wangys', 'wangwh', 'wuzs', 'yuanyang', 'jianxf', 'zhoumc', 'lilx']
                    ...
                    if (item['assignedTo'] in employees or item['resolvedBy'] in employees) and item['resolution'] not in invalidBug:  
                        bug_list.append({
                            ...
                            '状态': status_mapping.get(item['status'], item['status']),
                            ...
                            '解决者': item['resolvedBy'] and user_map.get(item['resolvedBy'].lower(), item['resolvedBy']),
                            '解决方案': solution_mapping.get(item['resolution'], item['resolution']),
...
if __name__ == "__main__":
    # 创建参数解析器
    parser = argparse.ArgumentParser(description='新签pod生产bug统计')
    ...
    title = '2026年6月15日-22日新签pod生产bug'
    ...
    with open(csv_name, 'w', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['编号', '标题', '链接', '状态', '创建日期', '指派日期',  '指派者', '创建者', '解决者','解决方案', '解决日期'])
        for item in allBug:
            writer.writerow([item['编号'], item['标题'], item['链接'], item['状态'], item['创建日期'], item['指派日期'], item['指派者'], item['创建者'], item['解决者'], item['解决方案'], item['解决日期']])
